...
```
/workspaces/codespaces-blank/hello.py
```

identifies a file named `hello.py`, which resides in a directory called `codespaces-blank`, which is itself in a directory 
called `workspaces`. The `workspaces` directory is located under the root, `/`.
...
```
/workspaces/codespaces-blank/Lab_1
```
