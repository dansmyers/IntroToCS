...
Python's `random` library includes a bunch of built-in functions for generating random numbers. One of its functions is `randint`, which generates a random integer from a specific range.

Put the following example in a file named `rand_test.py`. Run the program multiple times and verify that you get different numbers on each run.
...
Now write a program that asks the user to guess a hidden number (picked using `randint`) then reports how close the guess was to the true value. Here's an example of what the output should look like:
...
Put your program in a file named `guess.py`. Here is some code to help you get started:
...
# Generate the random target value
target = randint(1, 100)

# Read a guess from the user with input()

# Calculate the distance between target and the input value

# Print the distance

```

Tip: There is a bulit-in function called `abs` that will calculate the absolute value of its input. Use it to find the distance between the guess and the target value, like this:
