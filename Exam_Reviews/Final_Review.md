...
### Logic
1. What is the output of the following program? *A lot of people got this one wrong on the midterm. Double-review the definitions of the `or` and `and` operators*.
```
a = True
b = False
c = True
...
2. Are the two following statements logically equivalent? That is, do they produce the same result for all values of `a`, `b`, and `c`?
```
x = not (a and b) or not c
y = not (a and b and c)
```
Tip: make a truth table that has eight entries, one for each logical combination of the three variables.

### Functions

1. What is the output of the following program? Tip: look carefully at the function. Do you see a `return`?
```
def area_of_circle(radius):
    result = radius ** 2 * 3  # Use 3 for pi for calculation (lol)
...
3. What is the result produced by calling `weird(weird(19))`?
```
def weird(x):
    if x % 7 <= 3:
        return 2 * x + 1
...
7. Give the sequence of values printed by this loop.
```
nums = [2, 3, 5, 7, 11, 13, 15]
